...
3. 服务器每分钟检查一次 `main`。仅在该提交的 CI 成功后，部署器更新插件并调用 AstrBot 的插件重载接口。
4. 部署失败时恢复旧插件目录并再次重载旧版本。部署记录留在服务器。

通常合并后约 1–3 分钟生效。插件重载期间对应插件可能短暂暂停处理事件；NapCat 不退出 QQ，两个容器不重启。

**不要把凭据、QQ群消息、学号、SQLite 数据库或 `.env` 提交到仓库。** 线上数据保留在 AstrBot 的 `data/` 中。仓库维护者提交的插件代码会在生产服务器执行，应只授予可信任的人写入权限。
