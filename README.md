...
**不要把凭据、QQ群消息、学号、SQLite 数据库或 `.env` 提交到仓库。** 线上数据保留在 AstrBot 的 `data/` 中。仓库维护者提交的插件代码会在生产服务器执行，应只授予可信任的人写入权限。当前 GitHub 账号的私有仓库不支持分支保护，因此 GitHub 本身不会阻止直接推送；服务器会拒绝部署没有对应合并 PR 的提交。
