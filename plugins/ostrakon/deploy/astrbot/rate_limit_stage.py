...
def _is_group_reaction_notice(event: AstrMessageEvent) -> bool:
...
def _is_ostrakon_control_command(event: AstrMessageEvent) -> bool:
    text = event.get_message_str()
    if not isinstance(text, str):
        return False
    return text.strip() in {"/ostrakon status", "/ostrakon reset"}


def _is_request_event(event: AstrMessageEvent) -> bool:
    raw = getattr(event.message_obj, "raw_message", None)
    return bool(
        isinstance(raw, dict)
        and raw.get("post_type") == "request"
        and raw.get("request_type") in {"group", "friend"}
    )
...
class RateLimitStage(Stage):
    ...
    async def process(
        ...
        Reaction notice、请求事件与 OSTRAKON 管理命令属于实时管理信号，不应被普通群聊流量拖延。
        仅绕过 group_msg_emoji_like notice、request event 和明确的 /ostrakon 管理命令；
        普通消息仍按原策略限流。
        """
        if (
            _is_group_reaction_notice(event)
            or _is_ostrakon_control_command(event)
            or _is_request_event(event)
        ):
            return
    ...
    def _remove_expired_timestamps(
        ...
        Args:
            timestamps (Deque[datetime]): 当前会话的请求时间戳队列。
