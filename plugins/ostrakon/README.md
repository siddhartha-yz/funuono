...
AstrBot 4.27.3 的 `RateLimitStage` 位于插件 handler 之前。如果群聊达到 AstrBot
的会话限流阈值且策略为 `stall`，reaction 管理事件和 `/ostrakon status` /
`/ostrakon reset` 也会排队，从而造成明显的处罚或管理命令延迟。

仓库提供了一个针对 AstrBot 4.27.3 验证过的窄范围部署补丁：

```text
deploy/astrbot/rate_limit_stage.py
```

它只让 `group_msg_emoji_like` notice 和两个 Ostrakon 管理命令绕过聊天限流，
普通群消息仍使用 AstrBot 原有限流策略。安装方式与版本兼容说明见
`deploy/astrbot/README.md`。
...
ruff check main.py ostrakon tests deploy/astrbot/rate_limit_stage.py
pytest -q
```
